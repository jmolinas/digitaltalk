...
class BookingRepository extends BaseRepository
{
     ...
     * @return array
     */
    public function getUsersJobsHistory(User $cuser, Request $request)
    {
    ...
    /**
     * Not Immediate Response Builder.
     *
     * @param array $data
     * @return array
     */
    protected function validateData(array $data): array
    {
        ...
        if ($data['immediate'] == 'no') {
            switch (true) {
                case (isset($data['due_date']) && $data['due_date'] == ''):
                    $response['status'] = 'fail';
                    $response['message'] = "Du måste fylla in alla fält";
                    $response['field_name'] = "due_date";
                    return $response;

                case (isset($data['due_time']) && $data['due_time'] == ''):
                    $response['status'] = 'fail';
                    $response['message'] = "Du måste fylla in alla fält";
                    $response['field_name'] = "due_time";
                    return $response;

                case (!isset($data['customer_phone_type']) && !isset($data['customer_physical_type'])):
                    $response['status'] = 'fail';
                    $response['message'] = "Du måste göra ett val här";
                    $response['field_name'] = "customer_phone_type";
                    return $response;

                case (isset($data['duration']) && $data['duration'] == ''):
                    $response['status'] = 'fail';
                    $response['message'] = "Du måste fylla in alla fält";
    ...
    protected function certifiedDataBuilder(array $data): string
    {
        switch (true) {
    ...
    protected function dataforDataBuilder(Job $job): array
    {
        $datafor = [];
        switch ($job->gender) {
            case 'male':
    ...
    public function store($user, $data)
    {
        ...
        $data['certified'] = $this->certifiedDataBuilder($data);

        if ($const_type = isset(self::CUSTOMER_TYPE[$consumer_type])) {
            $data['job_type'] = $const_type;
        }
        ...
        $response['id'] = $job->id;
        $data['job_for'] = $this->dataforDataBuilder($job);
    ...
    protected function sendMail($email, $name, $subject, $data)
    {
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $this->sendMail($email, $name, $subject, $data);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $msg_contents = ($data['immediate'] == 'no') ? 'Ny bokning för ' . $data['language'] . 'tolk ' . $data['duration'] . 'min ' . $data['due'] :
     ...
     * @return mixed
     */
    public function updateJob(Job $job, array $data, User $user)
    {
        ...
        return ['Updated translator changed'];

        if ($langChanged) $this->sendChangedLangNotification($job, $old_lang);
        return ['Updated lang changed'];
    }
